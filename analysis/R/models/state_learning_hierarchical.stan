// Hierarchical State Learning Model
...
data {
  ...
  int<lower=0, upper = 2> choices[TotalTrials]; // all choices *** lower bound 0 to accommodate missed trials
  int<lower=0, upper = 2> activeRule[TotalTrials]; //activeRule on each trial
...
transformed parameters {
  real Q[TotalTrials]; // Q value - learned from reward
  vector[2] armQs[TotalTrials]; // vector of individual Q values after transfer
  ...
  for (t in 1:TotalTrials){
    if (TrialNum[t]==1){ // first trial for a given subject
      Q[t] = starting_utility; // initial expected utilities for both arms
    } else{
      Q[t] = Q[t-1]; // inherit previous values
      if (choices[t-1] != 0){ // if the previous trial was not a missed trial
        Q[t] = Q[t-1] + inv_logit(eta[Subject[t]]) * (activeRule[t-1] - 1 - Q[t-1]); //update q value
      }
    }
    armQs[t][1] = 1 - Q[t];
    armQs[t][2] = Q[t];
...
model {
  ...
  for (t in 1:TotalTrials){
    if (choices[t] != 0){ // adding this to avoid issues with missed trials
      target += log_softmax(beta[Subject[t]] * armQs[t])[choices[t]]; // the probability of the choices on each trial given utilities
    }
  }
}
...
generated quantities{
  ...
  for (t in 1:TotalTrials){
    if (choices[t] != 0){ // check for missing-choice trials
      raw_log_lik[t] = bernoulli_logit_lpmf(choices[t] - 1 | beta[Subject[t]] * (armQs[t][2]-armQs[t][1])); 
