...
parameters {
  real<lower = 0, upper = 1> kappa; //learning rate for Rewarded outcome
  real<lower = 0, upper = 1> eta; //associability weight
...
transformed parameters {
  ...
  for (trial in 1:nTrials) {
    ...
    Q[trial, ruleChoice[trial]] = Q[trial, ruleChoice[trial]] + kappa * alpha[trial] * delta[trial];
    
    // update alpha
    alpha[trial] = eta * fabs(delta[trial]) + (1 - eta) * alpha[trial];
  }
}
