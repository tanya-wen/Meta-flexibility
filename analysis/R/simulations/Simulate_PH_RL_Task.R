...
eta <- 0.1 # weighting factor that accounts for the gradual update in associability across trials
Qi <- 0.5 #initial Q values
currentAlpha <- 1 # alpha in current trial
...
trialAlphas <- vector(length = nTrials) #stores associability in each trial
...
for (trial in 1:nTrials) {
  ...
  delta <- (rewards[trial] - currentQs[choices[trial]])
    
  #given reward outcome, update Q values
  currentQs[choices[trial]] <- currentQs[choices[trial]] + kappa * currentAlpha * delta
  
  #save Q values in matrix of all Q-values for later visualization
  trialQs[trial,] <- currentQs
  
  # store alpha for this trial
  trialAlphas[trial] <- currentAlpha
  
  # update alpha
  currentAlpha <- eta * abs(delta) + (1 - eta) * currentAlpha
}
...
##### Plot Trial Alphas #####
alpha_df <- data.frame(matrix(unlist(trialAlphas), nrow=length(trialAlphas), byrow=T))

colnames(alpha_df)[1] <- "trialChoice"

alpha_df$trialCount <- as.numeric(row.names(alpha_df))

ggplot(data=alpha_df, aes(x = trialCount, y = trialAlphas)) +
  geom_point(size = 0.5) +
  geom_vline(xintercept = seq(0, nTrials, by=20)) +
  ggtitle("Trial Alpha")
