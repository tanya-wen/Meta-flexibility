...
for (subj in 1) {
  ...
  if (subject_total_acc[subj] >= 0.65) {
    ...
    model_data_learning = list(nSortingRules = 2,
                        ...
                        activeRule = sub_dat$activeRule[1:120])
    my_model_learning = stan_model(file = "RL_model_StateLearning.stan")
    ...
    model_data_transfer = list(nSortingRules = 2,
                       ...
                       activeRule = sub_dat$activeRule[122:240])
    my_model_transfer = stan_model(file = "RL_model_StateLearning.stan")
