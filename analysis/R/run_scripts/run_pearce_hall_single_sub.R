...
volatility = list();
sub_kappa_learning = list();
sub_eta_learning = list();
sub_beta_learning = list();
sub_kappa_transfer = list();
sub_eta_transfer = list();
...
for (subj in 1:length(subFiles)) {
  ...
  if (subject_total_acc[subj] >= 0.65) {
    ...
    my_model_learning = stan_model(file = "RL_PH_model.stan")
    fit_learning = optimizing(object = my_model_learning, data = model_data_learning)
    #get kappa and beta estimates
    sub_kappa_learning[subj] = fit_learning$par[1];
    sub_eta_learning[subj] = fit_learning$par[2];
    ...
    my_model_transfer = stan_model(file = "RL_PH_model.stan")
    fit_transfer = optimizing(object = my_model_transfer, data = model_data_transfer)
    #get kappa and beta estimates
    sub_kappa_transfer[subj] = fit_transfer$par[1];
    sub_eta_transfer[subj] = fit_transfer$par[2];
    sub_beta_transfer[subj] = fit_transfer$par[3];
  }
  sub_BIC_learning[subj] <- 3*log(120) - 2*fit_learning$value;
  sub_BIC_transfer[subj] <- 3*log(119) - 2*fit_transfer$value;
}
...
# group t-test
## learning phase ##
t.test(as.numeric(sub_kappa_learning[low_volaility_ind]), as.numeric(sub_kappa_learning[high_volaility_ind]))
t.test(as.numeric(sub_eta_learning[low_volaility_ind]), as.numeric(sub_eta_learning[high_volaility_ind]))
t.test(as.numeric(sub_beta_learning[low_volaility_ind]), as.numeric(sub_beta_learning[high_volaility_ind]))
## transfer phase ##
t.test(as.numeric(sub_kappa_transfer[low_volaility_ind]), as.numeric(sub_kappa_transfer[high_volaility_ind]))
t.test(as.numeric(sub_eta_transfer[low_volaility_ind]), as.numeric(sub_eta_transfer[high_volaility_ind]))
...
anova_dat <- data.frame(PID = rep(seq(from = 1, to = n_good_sub, by = 1), 2),
                       dat = as.numeric(c(sub_kappa_learning[low_volaility_ind],sub_kappa_learning[high_volaility_ind],sub_kappa_transfer[low_volaility_ind],sub_kappa_transfer[high_volaility_ind])),
