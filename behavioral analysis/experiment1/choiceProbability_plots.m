...
title({'Experiment 1','learning phase'})
set(gcf,'color','w')
box off
print(gcf,fullfile(fig_path,'ChoiceProb_Exp1learning.eps'),'-depsc2','-painters');
...
ylabel('choice probability')
xlabel('trials after rule change')
title({'Experiment 1','transfer phase'})
set(gcf,'color','w')
box off
print(gcf,fullfile(fig_path,'ChoiceProb_Exp1transfer.eps'),'-depsc2','-painters');
...
%% phase (learning vs. transfer) x group (high vs. low) x boundary (before vs. after)
anova_prob = [[mean(sub_ChoiceAcc_low_before(low_subs,:),2); mean(sub_ChoiceAcc_high_before(high_subs,:),2)], ...
    [mean(1-sub_ChoiceAcc_low_after(low_subs,:),2); mean(1-sub_ChoiceAcc_high_after(high_subs,:),2)], ...
...
% simple effect of boundary
before = [mean(sub_ChoiceAcc_low_before(low_subs,:),2); mean(sub_ChoiceAcc_high_before(high_subs,:),2);
    mean(sub_ChoiceAcc_before(low_subs,:),2); mean(sub_ChoiceAcc_before(high_subs,:),2)];
after = [mean(1-sub_ChoiceAcc_low_after(low_subs,:),2); mean(1-sub_ChoiceAcc_high_after(high_subs,:),2);
    mean(1-sub_ChoiceAcc_after(low_subs,:),2); mean(1-sub_ChoiceAcc_after(high_subs,:),2)];
[H,P,CI,STATS] = ttest(before,after)

% simple effect of phase
learning = [mean(sub_ChoiceAcc_low_before(low_subs,:),2); mean(sub_ChoiceAcc_high_before(high_subs,:),2);
    mean(1-sub_ChoiceAcc_low_after(low_subs,:),2); mean(1-sub_ChoiceAcc_high_after(high_subs,:),2)];
transfer = [mean(sub_ChoiceAcc_before(low_subs,:),2); mean(sub_ChoiceAcc_before(high_subs,:),2);
    mean(1-sub_ChoiceAcc_after(low_subs,:),2); mean(1-sub_ChoiceAcc_after(high_subs,:),2)];
[H,P,CI,STATS] = ttest(learning,transfer)

% simple effect of volatility
low = [mean(sub_ChoiceAcc_low_before(low_subs,:),2); mean(1-sub_ChoiceAcc_low_after(low_subs,:),2);
    mean(sub_ChoiceAcc_before(low_subs,:),2); mean(1-sub_ChoiceAcc_after(low_subs,:),2)];
high = [mean(sub_ChoiceAcc_high_before(high_subs,:),2); mean(1-sub_ChoiceAcc_high_after(high_subs,:),2);
    mean(sub_ChoiceAcc_before(high_subs,:),2); mean(1-sub_ChoiceAcc_after(high_subs,:),2)];
