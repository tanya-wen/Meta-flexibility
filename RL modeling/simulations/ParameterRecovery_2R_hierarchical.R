...
group1_alphas_P = c(0.5, 0.4, 0.3, 0.2, 0.1) # low-volatility group
group2_alphas_P = c(0.5, 0.6, 0.7, 0.8, 0.9) # high-volatility group
group1_alphas_N = c(0.5, 0.4, 0.3, 0.2, 0.1) # low-volatility group
group2_alphas_N = c(0.5, 0.6, 0.7, 0.8, 0.9) # high-volatility group


for (iter_P in 1:5) {
  print(paste('iter_P:',iter_P))
  
  for (iter_N in 1:5) {
    print(paste('iter_N:',iter_N))
    ...
    for (sub in 1:80) {
      ...
      if (sub <= 40) {
        alpha_P <- c(alpha_P, rnorm(1, mean = group1_alphas_P[iter_P], sd = 0.06)); #learning rate: sample from normal distribution
      } else if (sub > 40) {
        alpha_P <- c(alpha_P, rnorm(1, mean = group2_alphas_P[iter_P], sd = 0.06)); #learning rate: sample from normal distribution
      }
      if (sub <= 40) {
        alpha_N <- c(alpha_N, rnorm(1, mean = group1_alphas_N[iter_N], sd = 0.03)); #learning rate: sample from normal distribution
      } else if (sub > 40) {
        alpha_N <- c(alpha_N, rnorm(1, mean = group2_alphas_N[iter_N], sd = 0.03)); #learning rate: sample from normal distribution
      }
      beta <- c(beta, rnorm(1, mean = 4.39, sd = 0.27)); #inverse temperature: sample from normal distribution
      ...
      for (trial in 1:nTrials) {
        ...
        if (rewards[trial] == 1) {
          currentQs[choices[trial]] <- currentQs[choices[trial]] + alpha_P[sub] * (rewards[trial] - currentQs[choices[trial]])
        } else if (rewards[trial] == 0) {
          currentQs[choices[trial]] <- currentQs[choices[trial]] + alpha_N[sub] * (rewards[trial] - currentQs[choices[trial]])
        } 
    ...
    alpha_P_mu_sim <- c(alpha_P_mu_sim, mean(alpha_P[41:80])-mean(alpha_P[1:40]))
    low_alpha_P_sim <- c(low_alpha_P_sim,mean(alpha_P[1:40]))
    high_alpha_P_sim <- c(high_alpha_P_sim,mean(alpha_P[41:80]))
    alpha_N_mu_sim <- c(alpha_N_mu_sim, mean(alpha_N[41:80])-mean(alpha_N[1:40]))
    low_alpha_N_sim <- c(low_alpha_N_sim, mean(alpha_N[1:40]))
    high_alpha_N_sim <- c(high_alpha_N_sim, mean(alpha_N[41:80]))
    ...
    model_fit_RW <- stan(
      file = "models/two_rates_ParameterRecovery.stan",  # Stan program
    ...
    alpha_P_mu = rstan::extract(model_fit_RW, pars = 'alpha_P_mu', permuted = TRUE)$alpha_P_mu;
    alpha_N_mu = rstan::extract(model_fit_RW, pars = 'alpha_N_mu', permuted = TRUE)$alpha_N_mu;
    beta_mu = rstan::extract(model_fit_RW, pars = 'beta_mu', permuted = TRUE)$beta_mu;
    
    alpha_P_mu_fit <- c(alpha_P_mu_fit, mean(alpha_P_mu[,2])-mean(alpha_P_mu[,1]))
    low_alpha_P_fit <- c(low_alpha_P_fit,mean(alpha_P_mu[,1]))
    high_alpha_P_fit <- c(high_alpha_P_fit,mean(alpha_P_mu[,2]))
    alpha_N_mu_fit <- c(alpha_N_mu_fit, mean(alpha_N_mu[,2])-mean(alpha_N_mu[,1]))
    low_alpha_N_fit <- c(low_alpha_N_fit,mean(alpha_N_mu[,1]))
    high_alpha_N_fit <- c(high_alpha_N_fit,mean(alpha_N_mu[,2]))
    beta_mu_fit <- c(beta_mu_fit, mean(beta_mu[,2])-mean(beta_mu[,1]))
    
  } # end of N iteration loop
  
} # end of P iteration loop

# --------------------------------- #
# Compare simulated vs. fitted data #
# --------------------------------- #
agreement_alphas_P <- cor(alpha_P_mu_sim, alpha_P_mu_fit)
print(agreement_alphas_P)
plot(alpha_P_mu_sim, alpha_P_mu_fit, main="Correlation between simulated and fitted positive alpha differences", xlab="simulated", ylab="fitted")

agreement_alphas_N <- cor(alpha_N_mu_sim, alpha_N_mu_fit)
print(agreement_alphas_N)
plot(alpha_N_mu_sim, alpha_N_mu_fit, main="Correlation between simulated and fitted negative alpha differences", xlab="simulated", ylab="fitted")
...
save(alpha_P_mu_sim, alpha_P_mu_fit, alpha_N_mu_sim, alpha_N_mu_fit, beta_mu_sim, beta_mu_fit, file ="parameter_recovery.RData")
